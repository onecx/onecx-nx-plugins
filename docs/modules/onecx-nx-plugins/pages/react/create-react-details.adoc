...
= Create a Detail Component (React)
...
When a resource name is specified with the `--resource` option, the generator will preset the API class name to `<Resource>Api` used by the generated detail component. This should correspond to the created schema in OpenAPI file, see step before.
...
* `src/components/<feature>/<feature>-detail` +
  Directory for reusable UI components (header, form, dialogs, breadcrumbs)
* `<Resource>DetailsPage` +
  Name for the detail page component
* `/:id` +
...
This command will generate a new detail component *book-details* within the specified feature module with the necessary structure and configurations. With the presetted resource/entity name *Book*, the generator will also preset the API class name to *BookApi* used by the generated detail component. This should correspond to the created schema in OpenAPI file, see step before.
