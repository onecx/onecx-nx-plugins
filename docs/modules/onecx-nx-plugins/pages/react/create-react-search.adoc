...
The generator presets the API service name to `+<Resource>APIService+` used by the generated search page, derived from the feature name. This should correspond to the schema created in the OpenAPI file.

.Create a React search page with the following command
[source,bash]
----
nx generate <namespace>/nx-plugin:react-search <feature>
----
...
* `<Resource>APIService` +
  Name for the generated API service used by the search hook
* `use<Resource>Search` +
...
[source,bash]
----
nx generate @onecx/nx-plugin:react-search book
----

This command generates a new search page *book-search* within the *book* feature with the necessary structure and configurations. The generator presets the API service name to *BookAPIService* used by the generated search hook. This should correspond to the created schema in the OpenAPI file.
