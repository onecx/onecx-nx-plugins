...
The React search page fetches its data through a generated `+<Resource>APIService+`, called from the `+use<Resource>Search+` hook. This page describes how to map the search criteria to the API request, how the search endpoint is defined in the OpenAPI file, and how to provide mock data while no real backend is available.


[#action-s1]
== ACTION S1 - Map criteria to the search request
...
.ACTION S1 in `+use<Resource>Search.ts+`
[%collapsible]
====
[source, typescript]
----
const fetch<Resource>SearchResults = async (
  <resource>Service: <Resource>APIService,
...
[IMPORTANT]
====
After adjusting the OpenAPI specification, regenerate the typed API client so the changes are reflected in `+<Resource>APIService+`.
====
...
== Using the Mock API
Point your application's BFF base URL to the mock server (e.g. `\http://localhost:3000`) instead of the real backend, so the generated `+<Resource>APIService+` calls the mock endpoints. Adjust the configured BFF/proxy target of your React UI app accordingly.
