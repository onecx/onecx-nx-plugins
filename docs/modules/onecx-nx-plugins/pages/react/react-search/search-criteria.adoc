...
.ACTION S3 in `+<resource>-search.types.ts+`
[%collapsible]
====
[source, typescript]
----
// ACTION S3: Define/adjust the search criteria
export interface <Resource>SearchCriteria {
  changeMe: string;
}

// ACTION S3: Adjust the initial search criteria values
