...
File::      `<resource>-details.hook.spec.ts` (hook tests)
File::      `<resource>-details.page.spec.tsx` (page tests)

==== Tests for the book

[horizontal]
Directory:: `bookstore/src/pages/book/book-details`
File::      `book-details.hook.spec.ts`
File::      `book-details.page.spec.tsx`
...
.ACTION D11 in `+<resource>-details.hook.spec.ts+`
[%collapsible]
====
[source, javascript]
----
...
.ACTION D11 in `+<resource>-details.page.spec.tsx+`
[%collapsible]
====
[source, javascript]
----
...
[source, javascript]
----
const updateDetails = useCallback(async (updatedDetails: ResourceDetails) => {
  ...
  try {
    ...
    // The API expects: updateResourceById(id: string, updateRequest: UpdateResourceRequest)
    // You may need to transform updatedDetails to match the UpdateRequest structure
    await resourceService.updateResourceById(
      details.id,
