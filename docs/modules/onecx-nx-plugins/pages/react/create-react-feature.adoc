...
* `<Feature>Feature` +
  Name for the generated feature entry component
* `/<feature>/` +
...
It will add a new route to `src/router.tsx` that renders the generated feature entry page component when the specified route is accessed. The generated feature page is intentionally only a placeholder and is typically replaced later by other generators such as the search generator. +
...
=== Routing
The generator will add a new route to `src/router.tsx` that renders the generated `BookFeature` placeholder page when the `/book/` route is accessed. This placeholder is later replaced by generators such as `react-search`. +
