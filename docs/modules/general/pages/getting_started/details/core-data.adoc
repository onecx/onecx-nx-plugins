...
The details page will reuse the created xref:getting_started/explanations.adoc#resource[`Resource`]. If the search-result object differs from the details object, you can **define a separate xref:getting_started/explanations.adoc#feature[`<Feature>Details`] object in** the **OpenAPI** specification and modify the endpoints accordingly (get, put, delete). You also need to modify the used xref:getting_started/explanations.adoc#resource[`Resource`] in the state, viewmodel, etc. to use it.
